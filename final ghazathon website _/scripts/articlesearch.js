let info = {
    data: [
        {
            Title: "Title lol", image: "../imgs/top_pic.jpg", 
            category1: "woman", category2: "60+", id:"1",
            Description:"amma",
            Content:"slay",
        },
        {
...
for (let i of info.data) {
    ...
    link.setAttribute("href", "../HTML/article.html");  // Use the link from the data
...
document.querySelectorAll('.video-link').forEach(link => {
    link.addEventListener('click', function (event) {
        const articleID = event.currentTarget.dataset.id;  // Get the id from data-id
        const selectedArticle = info.data.find(article => article.id === articleID);
